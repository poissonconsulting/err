test_that("cc_internal", {
  ...
  expect_identical(
    cc_internal(1:3, bracket = "'", ellipsis = 10),
    "'1', '2', '3'"
  )
  expect_identical(cc_internal(1:4, ellipsis = 4, bracket = ""), "1, 2, ..., 4")
  expect_identical(
    cc_internal(5:1, ellipsis = 4, bracket = "'"),
    "'5', '4', ..., '1'"
  )
  expect_identical(
    cc_internal(c(1, 2:3, 4), ellipsis = 2, bracket = "'"),
    "'1', '2', ..., '4'"
  )
  expect_identical(
    cc_internal(1:3, ellipsis = 1, bracket = "'"),
    "'1', '2', '3'"
  )
})
...
test_that("cc", {
  expect_identical(
    cc(c(1, 2:4, 5), conjunction = "or", oxford = TRUE, ellipsis = 5),
    "1, 2, 3, ..., or 5"
  )
  expect_identical(
    cc(1:6, conjunction = "and", ellipsis = 5),
    "1, 2, 3, ... and 6"
  )
  expect_identical(
    cc(as.character(1:6), conjunction = "and", ellipsis = 5),
    "'1', '2', '3', ... and '6'"
  )
...
test_that("cc.data.frame", {
  ...
  expect_identical(
    cc(datasets::mtcars, "and", ellipsis = 3),
    "mpg: 21, 21, ... and 21.4\ncyl: 6, 6, ... and 4\n... and carb: 4, 4, ... and 2"
  )
