test_that("co_sub", {
  expect_identical(
    co_sub("%c | %n %o%s", 0:1, "obj", bracket = "'"),
    "'0', '1' | 2 objs"
  )
...
test_that("co or", {
  expect_identical(
    co(1:3, "the %n things: %c", conjunction = "or"),
    "the 3 things: 1, 2 or 3"
  )
  expect_identical(
    co(factor(1:3), "the %n things: %c", oxford = TRUE, conjunction = "or"),
    "the 3 things: '1', '2', or '3'"
  )
  expect_identical(
    co(1:3, "the %n things: %c of %o", bracket = "'", conjunction = "or"),
    "the 3 things: '1', '2' or '3' of 1:3"
  )
})

test_that("co and", {
  expect_identical(
    co(1:3, "the %n things: %c", conjunction = "and"),
    "the 3 things: 1, 2 and 3"
  )
  expect_identical(
    co(1:3, "the %n things: %c", oxford = TRUE, conjunction = "and"),
    "the 3 things: 1, 2, and 3"
  )
})
