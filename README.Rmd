...
`err` is an R package to produce customizable number and object sensitive error and warning messages.
...
The `co` and `cs` functions can be combined with the wrappers `msg`, `wrn` and `err` to produce a message, warning and error (without the call as part of the warning/error message).
